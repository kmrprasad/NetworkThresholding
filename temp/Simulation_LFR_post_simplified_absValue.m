...
FC_sim_abs = zeros(numberNodes,numberNodes,numberSim);
...
CC_FCsim_abs = zeros(numberNodes,numberSim);
...
NMI_sim_header={'Weighted','Absolute Value of Weighted','Percolation','Objective Lambda',...
...
for h = 1:numberSim
    ...
    FC_sim(:,:,h) = RealCorMat - diag(diag(RealCorMat)); %zero out the diagonal
   
    FC_sim_abs(:,:,h) = abs(FC_sim(:,:,h));
    ...
    modules_estimate_2 = modularity_und(FC_sim_abs(:,:,h));
    ...
    weightedEdgeVector = Adj2lowerTriangleVector(FC_sim(:,:,h)); %get unique lower triangle off-diagonal elements
    weightedEdgeVector=round(weightedEdgeVector,3); %round to the nearest thousandth
    roundedRank_threshold_space = abs(unique(weightedEdgeVector)); %remove duplicates & does absolute value
...
%   fixed_threshold_space = linspace(0,1,41);
    ...
    [FC_sim_perc(:,:,h),threshold_value_Perc(h)] = threshold_percolation_depricated(FC_sim_abs(:,:,h),roundedRank_threshold_space);
    ...
    [FC_sim_objL(:,:,h),threshold_value_ObjL(h)] = threshold_objective_LAMBDA(FC_sim_abs(:,:,h),roundedRank_threshold_space,GCC);
    ...
    disp('max tree');
    FC_sim_tree(:,:,h) = UndirectedMaximumSpanningTree(FC_sim_abs(:,:,h));
...
CC_FCsim_abs(:,h) =  clustering_coef_wu( FC_sim_abs(:,:,h));
...
d_FCsim_abs(:,h) =  density_und( FC_sim_abs(:,:,h));
...
density=[d_FCsim',d_FCsim_abs',d_FC_perc',d_FC_objL',d_FC_stat',d_FC_tree'];
...
figure; hold on;
plot(mean(CC_FCsim,2),'Color',[.4 .4 .4],'LineWidth',5)
plot(mean(CC_FCsim_abs,2),'Color',[.8 .8 .8],'LineWidth',4) % r for red
plot(mean(CC_FC_perc,2),'Color','r','LineWidth',5) % r for red
plot(mean(CC_FC_objL,2),'Color','b','LineWidth',4) % r for red
plot(mean(CC_FC_stat,2),'Color',[.6 .2 .6],'LineWidth',4) % r for red
plot(mean(CC_FC_tree,2),'Color',[.2 .8 .2],'LineWidth',5) % r for red
lgd=legend(NMI_sim_header);
...
Edges_FCsim = zeros(numberEdges,numberSim);
